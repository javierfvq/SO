...
void printMessageInitialization(Island_CONFIG *island) {
    char *buffer;
    asprintf(&buffer, "Island %s initialized.\n", island->name);
    printMessage(buffer);
    free(buffer);
    if (island->max_capacity== 1) {
         asprintf(&buffer, "Port capacity: %d ship.\n", island->max_capacity);
        printMessage(buffer);
        free(buffer);
    } else if (island->max_capacity > 1) {
        asprintf(&buffer, "Port capacity: %d ships.\n", island->max_capacity);
        printMessage(buffer);
        free(buffer);
    }
  
    asprintf(&buffer, "%d sea routes loaded.\n", island->counter);
    printMessage(buffer);
    free(buffer);

    asprintf(&buffer, "%d products available.\n", island->product_count);
    printMessage(buffer);
    free(buffer);
