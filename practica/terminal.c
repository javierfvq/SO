...
int  detectIsMissingAField (int commandType) {
    ...
    char *cuarta_palabra = strtok(NULL, " ");
   switch (commandType) {
      ...
      case LIST:
         if (segunda_palabra == NULL  || tercera_palabra != NULL) {
            return INCORRECT;
         } else if (strcmp(segunda_palabra, "voyages") == 0) {
            return CORRECT;
         }else if (strcmp(segunda_palabra, "market") == 0) {
            return CORRECT;
         } else {
            return INCORRECT;
         } 
      ...
      case ACCEPT:
         if (segunda_palabra == NULL || atoi_personalizado (segunda_palabra) == -1 || tercera_palabra != NULL) {
            return INCORRECT;
         }else{
            return CORRECT;
         }
      ...
      case BUY:
          if (segunda_palabra == NULL || tercera_palabra == NULL || atoi_personalizado(tercera_palabra) == -1 || cuarta_palabra != NULL) {
              return INCORRECT;
          }else{
              return CORRECT;
          }
...
void DoAction(int commandType){
  ...
  correct = detectIsMissingAField(commandType);
    ...
    switch (commandType) {
        ...
        case LIST:
            if (correct == -1) {
           printError("Usage: LIST VOYAGES / LIST MARKET\n");
        } else {
            printMessage("Command OK\n");
        } 
          break;
        case ACCEPT:
            if (correct == -1) {
                printError("Usage: ACCEPT <voyage_id>\n");
            } else {
                printMessage("Command OK\n");
            } 
            break;
